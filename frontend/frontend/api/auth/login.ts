...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const { email, personalCode, verificationCode, code } = req.body || {};
  const clean = String(email || "").trim().toLowerCase();
  const provided = String(personalCode ?? verificationCode ?? code ?? "");
  if (!clean || !provided) return res.status(400).json({ error: "missing_params" });

  const db = await getDb();
  const lc = await db.collection("loginCodes").findOne({
    email: clean, code: provided, usedAt: null, expiresAt: { $gt: new Date() }
  });
